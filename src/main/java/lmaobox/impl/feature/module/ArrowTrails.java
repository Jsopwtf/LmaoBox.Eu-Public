...
public class ArrowTrails extends Module {
    ...
    public ArrowTrails() {
        super(Lmaobox.ADD, "arrows-trails", "");
    }
    ...
    Setting<Integer> points = General.add(new IntSetting.Builder()
        .name("max-points")
        .defaultValue(150)
        .min(1)
    ...
    Setting<colorizeMode> colorMode = General.add(new EnumSetting.Builder<colorizeMode>()
        .name("color-mode")
        .defaultValue(colorizeMode.Random)
        .build()
    );

    Setting<SettingColor> colorSetting = General.add(new ColorSetting.Builder()
        .name("trail-color")
    ...
    void a(Render3DEvent e) {
        ...
        mc.world.getEntities().forEach(entity -> {
            if (entity instanceof PersistentProjectileEntity arrow && !inGround(arrow)) {
